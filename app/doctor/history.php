...
<link rel="stylesheet" type="text/css" href="http://localhost/smart_healthcare/fontawesome/css/all.css">
       ...
       <link rel="shortcut icon" href="http://localhost/smart_healthcare/images/logo1.png" type="image/ico" />
        ...
        <title>Patient History | Smart Healthcare</title>
...
{
$sql=mysqli_query($con,"select a.P_id,a.C_id,Profile,F_Name,Specialist,Date from doctor d,appoinment a where  d.D_id=a.D_id and a.P_id='".$_GET['pid']."' and a.Date<='$currentTime' and a.Payment is not null and Payment_status='Paid'");
}
while($data=mysqli_fetch_array($sql))
{
?>
                    <a href="history.php?pid=<?php echo $data['P_id']?>&cid=<?php echo $data['C_id']?>">
...
if(isset($_GET['pid']) &&!empty($_GET['pid']) AND  isset($_GET['cid']) &&!empty($_GET['cid']))
		  {
		          $sql1=mysqli_query($con,"select F_Name,profile,Name,r_tital,r_description,p_tital,pdf from report r, laboratory l where r.L_id=l.L_id and C_id = '".$_GET['cid']."'  and P_id='".$_GET['pid']."'");
		  }
				  while($data1=mysqli_fetch_array($sql1))
				  {
...
?>
                                ...
                                <object data='<?php echo $image_src;  ?>' type="application/pdf" onerror=this.src="http://localhost/smart_healthcare/images/def.jpg" alt="not " width="100%" height="460px">
...
?>
 ...
 <img src='<?php echo $image_src;  ?>' onerror="this.src='http://localhost/smart_healthcare/images/def2.JPG';" alt="fgdfgdfg"/>
