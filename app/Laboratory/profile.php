...
if(isset($_POST['submit']))
{
   ...
   $contact=$_POST['phone'];
   $sql=mysqli_query($con,"Update laboratory set F_Name='$fname',L_Name='$lname',Address='$address',City='$city',Pincode='$pincode',Contect='$contact',updationDate='$currentTime' where L_id='".$_SESSION['id']."'");
   if($sql)
   {
...
<html>
	...
	<link rel="stylesheet" type="text/css" href="http://localhost/smart_healthcare/fontawesome/iconic/css/material-design-iconic-font.min.css">
       ...
       <script src="http://localhost/smart_healthcare/css/cssall/js/demo.js" type="text/javascript"></script>
        ...
        <title>Laboratory Profile | Smart Healthcare</title>
...
else
{
 ...
 $micro=round(microtime(true) * 1000);
		...
		$result =mysqli_query($con,"SELECT Profile FROM laboratory WHERE Profile='$pro'");
...
else
{
        ...
        if( in_array($imageFileType,$extensions_arr) ){
            ...
            else
			{
                 ...
                 $query = "Update laboratory set Profile='$pro',updationDate='$currentTime' where L_id='".$_SESSION['id']."'";
...
<div id="div1" style="color:red; font-size:15px; "><?php echo htmlentities($_SESSION['msgpro']); ?></div>
	 ...
	 <div class="box-content">
		...
		<?php $sql=mysqli_query($con,"select * from laboratory where Email='".$_SESSION['login']."'");
