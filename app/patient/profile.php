...
$currentTime = date( 'yy-M-d h:i:s A', time () );
if(isset($_POST['submit']))
{
   ...
   $email=$_POST['email'];
   $contact=$_POST['phone'];
   $sql=mysqli_query($con,"Update patient set F_Name='$fname',L_Name='$lname',Address='$address',City='$city',Pincode='$pincode',Email='$email',Phone='$contact',updationDate='$currentTime' where P_id='".$_SESSION['id']."'");
   if($sql)
   {
...
<html>
	...
	<link rel="stylesheet" type="text/css" href="http://localhost/smart_healthcare/fontawesome/iconic/css/material-design-iconic-font.min.css">
       ...
       <script src="http://localhost/smart_healthcare/css/cssall/js/demo.js" type="text/javascript"></script>
        ...
        <title>Patient Profile | Smart Healthcare</title>
...
$micro=round(microtime(true) * 1000);
		...
		$result =mysqli_query($con,"SELECT Profile FROM patient WHERE Profile='$pro'");
...
else
{
        ...
        if( in_array($imageFileType,$extensions_arr) ){
            ...
            else
			{
                 ...
                 $query = "Update patient set Profile='$pro',updationDate='$currentTime' where P_id='".$_SESSION['id']."'";
...
<div id="div1" style="color:red; font-size:15px; "><?php echo htmlentities($_SESSION['msgpro']); ?></div>
	 ...
	 <div class="box-content">
		...
		<?php $sql=mysqli_query($con,"select * from patient where P_id='".$_SESSION['id']."'");
...
<img src='<?php echo $image_src;  ?>' onerror=this.src="http://localhost/smart_healthcare/images/def.jpg">
					 ...
					 <div class="wrap-input10 validate-input" data-validate = "Enter valid Phone No">
						<input style="color:#000000" class="input100" type="text" placeholder="Phone" name="phone" tabindex=8  value="<?php echo htmlentities($data['Phone']);?>"/>
						<span class="focus-input100"></span>
					</div>
