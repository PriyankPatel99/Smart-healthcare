...
if(isset($_POST['submit']))
{
   ...
   $email=$_POST['email'];
   $contact=$_POST['phone'];
   $sql=mysqli_query($con,"Update hospital set F_Name='$fname',L_Name='$lname',Gr_no='$grno',Name='$name',Email='$email',Contect='$contact',updationDate='$currentTime' where H_id='".$_SESSION['id']."'");
   if($sql)
   {
...
<html>
	...
	<link rel="stylesheet" type="text/css" href="http://localhost/smart_healthcare/fontawesome/iconic/css/material-design-iconic-font.min.css">
       ...
       <script src="http://localhost/smart_healthcare/css/cssall/js/demo.js" type="text/javascript"></script>
        ...
        <title>Hospital Profile | Smart Healthcare</title>
...
else
{
		...
		$result =mysqli_query($con,"SELECT Profile FROM hospital WHERE Profile='$pro'");
...
else
{
        ...
        if( in_array($imageFileType,$extensions_arr) ){
            ...
            else
			{
                 ...
                 $query = "Update hospital set Profile='$pro',updationDate='$currentTime' where H_id='".$_SESSION['id']."'";
...
<div id="div1" style="color:red; font-size:15px; "><?php echo htmlentities($_SESSION['msgpro']); ?></div>
	 ...
	 <div class="box-content">
		...
		<?php $sql=mysqli_query($con,"select * from hospital where H_id='".$_SESSION['id']."'");
...
<img src='<?php echo $image_src;  ?>' onerror=this.src="http://localhost/smart_healthcare/images/def.jpg">
					 ...
					 <div class="wrap-input1 validate-input" data-validate = "minimum 3 Disit Alphabetic First Name">
						<input style="color:#000000" class="input100" type="text" name="F_Name" placeholder="MD First Name" id="name" tabindex=3 value="<?php echo htmlentities($data['F_Name']);?>"/>
                         <span class="focus-input100"></span>
						</div>
					  <div class="wrap-input2 validate-input" data-validate = "minimum 3 Disit Alphabetic  Last_Name">
						<input style="color:#000000" class="input100" type="text" placeholder="MD Last Name" name="L_Name" id="name" tabindex=4  value="<?php echo htmlentities($data['L_Name']);?>"/>
                        <span class="focus-input100"></span>

					</div>
						 <div class="wrap-input100 validate-input" data-validate = "Enter valid Gr.no">
						<input style="color:#000000" class="input100" type="text" placeholder="Gr_no" name="grno" tabindex=5 value="<?php echo htmlentities($data['Gr_no']);?>"/>
						<span class="focus-input100"></span>
					</div>
					     <div class="wrap-input100 validate-input" data-validate = "minimum 3 Disit Alphabetic Hospital Name">
						<input style="color:#000000" class="input100" type="text"  placeholder="Hospital Name" name="name" id="name" tabindex=6 value="<?php echo htmlentities($data['Name']);?>"/>
						<span class="focus-input100"></span>
						</div>
				 <div class="wrap-input3 validate-input" data-validate = "Enter valid Email">
						...
						<input style="color:#000000" class="input100" type="text" placeholder="Phone" name="phone" tabindex=7  value="<?php echo htmlentities($data['Contect']);?>"/>
						<span class="focus-input100"></span>
